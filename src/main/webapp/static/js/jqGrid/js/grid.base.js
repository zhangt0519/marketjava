...
(function($){
	$.fn.wresize = function(f){
		...
		//IE6下辅助触发resize @ by yemoo 2010-11-3
		if($.browser.msie && $.browser.version == '6.0'){
			var doc = $(window), width = doc.width(), height = doc.height();
...
$.fn.jqGrid = function( pin ) {
	...
	return this.each( function() {
		...
		setPager = function (pgid, tp){
			...
			$("#"+pgid)
			.append("<div id='"+pgcnt+"' class='ui-pager-control' role='group'><table cellspacing='0' cellpadding='0' border='0' class='ui-pg-table' style='width:100%;table-layout:fixed;height:100%;' role='row'><tbody><tr><td id='"+lft+"' align='left' class='tool-page-left'></td><td id='"+cent+"' align='center' style='white-space:pre;'></td><td id='"+rgt+"' align='right'></td></tr></tbody></table></div>")
			...
			if(ts.p.pgbuttons===true) {
				...
				pgl += "<td id='"+po[0]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon-seek-first'>首页</span></td>";
				pgl += "<td id='"+po[1]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon-seek-prev'>上一页</span></td>";
				pgl += pginp !== "" ? sep+pginp+sep:"";
				pgl += "<td id='"+po[2]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon-seek-next'>下一页</span></td>";
				pgl += "<td id='"+po[3]+"' class='ui-pg-button ui-corner-all'><span class='ui-icon-seek-end'>尾页</span></td>";
		...
		if(ts.p.autowidth===true) {
            ...
            var calcWidth = function(){
                ...
                var gridCtn = $(eg).width('');
                outerWidth = gridCtn.outerWidth()-gridCtn.innerWidth();
                //alert(outerWidth);
                var inner = gridCtn.find('.ui-jqgrid-view,.ui-jqgrid-pager').hide();
                var width = isIE6 ? (gridCtn.parent().innerWidth() - outerWidth) : gridCtn.innerWidth();
                inner.show();
               // alert(width);
                gridCtn.width(width);
            ...
            //IE6特殊处理
            if($.browser.msie && $.browser.version == '6.0'){
                var outerWidth = $(eg).outerWidth()-$(eg).innerWidth();
               // alert(outerWidth);
                ts.p.width = $(eg).parent().innerWidth() - outerWidth;
                calcWidth()
                setTimeout(calcWidth,0);
            }
            //resizegrid @by kan 2011-03-09
           // $(window).resize(calcWidth);
            /*
             $(window).wresize(function(){
                ...
                var inner = gridCtn.find('.ui-jqgrid-view,.ui-jqgrid-pager').hide();
                inner.show();
                var  tbdWidth=$(document.body).width()+outerWidth;
                gridCtn.width(tbdWidth);
                _this.setGridWidth(tbdWidth);
                 //alert();
             }) ;
           // $(window).wresize(calcWidth);
           */
